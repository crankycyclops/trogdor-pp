...
TEST_SUITE("CreatureController (scopes/creature.cpp)") {

	TEST_CASE("CreatureController (scopes/creature.cpp): getEntity()") {
		...
		for (const auto &entity: entities) {
			...
			response = getEntity(gameId, entity.c_str(), "creature");

			if (creatures.end() != creatures.find(entity)) {
		...
		// The player should NOT be a creature
		response = getEntity(gameId, playerName, "creature");
	...
	TEST_CASE("CreatureController (scopes/creature.cpp): getEntityList()") {
		...
		response = getEntityList(gameId, "creature");
		...
		for (auto i = response["entities"].Begin(); i != response["entities"].End(); i++) {
			...
			bool entityValid =
				creatures.end() != creatures.find((*i)["name"].GetString()) ||
