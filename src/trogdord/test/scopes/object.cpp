...
TEST_SUITE("ObjectController (scopes/object.cpp)") {

	TEST_CASE("ObjectController (scopes/object.cpp): getEntity()") {
		...
		for (const auto &entity: entities) {
			...
			response = getEntity(gameId, entity.c_str(), "object");

			if (objects.end() != objects.find(entity)) {
		...
		// The player should NOT be a object
		response = getEntity(gameId, playerName, "object");
	...
	TEST_CASE("ObjectController (scopes/object.cpp): getEntityList()") {
		...
		response = getEntityList(gameId, "object");
		...
		for (auto i = response["entities"].Begin(); i != response["entities"].End(); i++) {
			...
			bool entityValid =
				objects.end() != objects.find((*i)["name"].GetString()) ||
