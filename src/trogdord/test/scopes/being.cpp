...
TEST_SUITE("BeingController (scopes/being.cpp)") {

	TEST_CASE("BeingController (scopes/being.cpp): getEntity()") {
		...
		for (const auto &entity: entities) {
			...
			response = getEntity(gameId, entity.c_str(), "being");

			if (beings.end() != beings.find(entity)) {
		...
		// The player should also be a being
		response = getEntity(gameId, playerName, "being");
	...
	TEST_CASE("BeingController (scopes/being.cpp): getEntityList()") {
		...
		response = getEntityList(gameId, "being");
		...
		for (auto i = response["entities"].Begin(); i != response["entities"].End(); i++) {
			...
			bool entityValid =
				beings.end() != beings.find((*i)["name"].GetString()) ||
