...
TEST_SUITE("RoomController (scopes/room.cpp)") {

	TEST_CASE("RoomController (scopes/room.cpp): getEntity()") {
		...
		for (const auto &entity: entities) {
			...
			response = getEntity(gameId, entity.c_str(), "room");

			if (rooms.end() != rooms.find(entity)) {
		...
		// The player should NOT be a room
		response = getEntity(gameId, playerName, "room");
	...
	TEST_CASE("RoomController (scopes/room.cpp): getEntityList()") {
		...
		response = getEntityList(gameId, "room");
		...
		for (auto i = response["entities"].Begin(); i != response["entities"].End(); i++) {
			...
			bool entityValid =
				rooms.end() != rooms.find((*i)["name"].GetString()) ||
